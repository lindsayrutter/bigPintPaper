...
\SetKwBlock{Fnb}{\textnormal{output\$litrePlot $\leftarrow$ renderPlotly(\{ }}{}
\Fnb{
\BlankLine
/* \textit{Create reactive expression of plotly background litre plot}\;
plotlyLitre $\leftarrow$ reactive(gP())\;
\BlankLine
/* \textit{Tailor interactivity of the plotly litre plot object using custom JavaScript}\;
\SetKwBlock{Fnc}{\textnormal{plotlyLitre() \%$>$\% onRender("function(el, x, data)\{ }}{}
\Fnc{
\BlankLine
...
\BlankLine
/* \textit{If the user changes their input, store information about new superimposed genes with a handle called `points'. These values can then be sent from R to JavaScript}\;
...
/* \textit{If the user changes their input, store information about new superimposed genes with a handle called `lines'. These values can then be sent from R to JavaScript}\;
...
\Fna{
\BlankLine
/* \textit{Shiny user input options}\;
\BlankLine
...
\KwData{Data frame input by user}
\KwResult{Interactive parallel coordinate plot}
...
\BlankLine
/* \textit{Create an empty ggplotly object with same dimensions as data}\;
p $\leftarrow$ ggplot(); gp $\leftarrow$ ggplotly(p)

\BlankLine
/* \textit{Send user-defined line color into onRender() function}\;
session\$sendCustomMessage(type = ``lines'', message = list(lineColor = lineColor))

\BlankLine
/* \textit{Declare Shiny output parallel coordinate plot}\;
\SetKwBlock{Fnb}{\textnormal{output\$pcpPlot $\leftarrow$ renderPlotly(\{ }}{}
\Fnb{

\BlankLine
/* \textit{Tailor interactivity of plotly parallel coordinate plot using custom JavaScript}\;
\SetKwBlock{Fnc}{\textnormal{gp \%$>$\% onRender(``function(el, x, data)\{ }}{}
\Fnc{

\BlankLine
/* \textit{Define rects array object to hold IDs of remaining lines}\;
var rects = []

\BlankLine
/* \textit{Draw lines for the original dataset}\;
...
\BlankLine
/* \textit{If the user draws a rectangle}\;
\SetKwBlock{Fnd}{\textnormal{el.on(`plotly\_selected', function(e)\{ }}{}
\Fnd{

\BlankLine
/* \textit{Delete lines outside the user-defined rectangle}\;
Traces.push(delLine)\;
Plotly.addTraces(el.id, Traces)\;

\BlankLine
/* \textit{Create new object called pcpDat that only contains genes within rectangle}\;

\BlankLine
/* \textit{Push IDs of remaining lines to rects array object}\;
for (a=0; a$<$pcpDat.length; a++)\{rects.push(pcpDat[a][`ID'])\}


\BlankLine
/* \textit{Save remaining gene IDs in an object called rects with handle called `rects' so it can be read outside current JavaScript function back in Shiny}\;
...
\BlankLine
/* \textit{Read into Shiny the gene IDs that remain within rectangle drawn by user}\;
inputRectDf $\leftarrow$ reactive({input\$rects})

\BlankLine
/* \textit{Print the selected gene IDs in interactive data table}\;
output\$rectdf = renderDataTable(pcpDat \%$>$\% filter(ID \%in\% inputRectDf()))


}\}\;
\caption{Pseudocode for interactive parallel coordinate plot}
\label{algorithmPCP}
\end{algorithm}
