...
This mechanism may have originally been designed for time series data, but it has since proven useful for detecting patterns in gene expression data. Combined with parallel coordinate plots, the consecutive box selection technique has been used to elicit candidate regulatory splice sequences showing high values at one position and low values at another position \cite{hochheiser2003dynamic}. In RNA-seq, this technology can also be used to investigate differential expression showing high read counts for one treatment group and low read counts for another treatment group, requiring a consecutive query. This technique has been published before in software that was constricted for certain operating systems \cite{hochheiser2003dynamic}. We are unaware of any published visualization techniques in R that allow for consecutive box selection and believe that publishing such tools in a platform available across operating systems can be useful for computational biologists. See Table~\ref{table:table2} for more details about how we incorporated this technique in our interactive parallel coordinate plots.
...
\caption{Resources for example interactive graphics in bigPint}
      \begin{tabular}{lllll}
        \hline
        Plot & Video explanation & Interactive application & Pseudocode & Code\\ \hline
        Scatterplot matrix & Figure a1 & https://bigpint.shinyapps.io/smplot/ & Figure a2 & Link \\
        Litre plot & Figure b1 & https://bigpint.shinyapps.io/litre/ & Figure b2 & Link\\
        Volcano plot & Figure c1 & https://bigpint.shinyapps.io/volcano/ & Figure c2 & Link\\
        Parallel coordinate plot & Figure d1 & https://bigpint.shinyapps.io/pcplot/ & Figure d2 & Link\\ \hline
      \end{tabular}
      \label{table:table2}
\end{table}
